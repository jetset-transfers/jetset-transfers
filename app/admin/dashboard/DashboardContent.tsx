    ...
    <div className="bg-navy-900 rounded-xl border border-navy-800 p-6">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="p-3 bg-navy-800 rounded-lg">
          ...
          <p className="text-lg font-bold text-green-400">${data.monthlyRevenue.toLocaleString()}</p>
          <p className="text-xs text-navy-500">{data.monthlyBookings} reservas</p>
        </div>
        <div className="p-3 bg-navy-800 rounded-lg">
          <p className="text-xs text-navy-400">Últimos 30 días</p>
          <p className="text-lg font-bold text-white">${data.totalRevenue.toLocaleString()}</p>
          <p className="text-xs text-navy-500">{data.totalBookings} reservas</p>
        </div>
        <div className="p-3 bg-navy-800 rounded-lg">
          <p className="text-xs text-navy-400">Promedio</p>
          <p className="text-lg font-bold text-brand-400">${data.averageBooking.toLocaleString()}</p>
          ...
          {data.chartData.map((day) => {
            ...
            return (
              <div
                ...
                title={`${date.toLocaleDateString('es-MX', { day: 'numeric', month: 'short' })}: $${day.amount} USD (${day.count} reservas)`}
              >
                <div
                  className={`w-full rounded-t transition-all ${
                    day.amount > 0
                      ? isToday ? 'bg-green-400' : 'bg-green-500/60 hover:bg-green-400'
                      : 'bg-navy-800'
                  }`}
...
const SYSTEM_INFO = {
  ...
  codeName: 'Jetset Transfers Admin',
...
export default function DashboardContent({ user, stats }: DashboardContentProps) {
  ...
  const quickActions = [
    {
      title: 'Destinos',
      description: 'Gestiona traslados',
      icon: MapPinIcon,
      color: 'text-blue-400',
